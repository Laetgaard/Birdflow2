...
function TabsComponent({ props, styles, isSelected, onClick, isPreview }: ComponentRenderProps) {
  const [activeTab, setActiveTab] = useState<string>(props.defaultTab || props.tabs?.[0]?.id || '1');
  const baseStyle = getBaseStyle(styles, isSelected, isPreview);
  ...
  return (
    <section style={baseStyle} onClick={onClick}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={getTabListStyle()} role="tablist" data-testid="tabs-list">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              role="tab"
        ...
        <div
          ...
          style={{
            padding: '24px',
            backgroundColor: tabStyle === 'boxed' ? '#ffffff' : 'transparent',
            border: tabStyle === 'boxed' ? `1px solid ${accentColor}` : 'none',
            borderTop: tabStyle === 'boxed' ? 'none' : undefined,
            borderRadius: tabStyle === 'boxed' ? '0 0 8px 8px' : '0',
            minHeight: '100px',
          }}
          data-testid="tabs-content-panel"
        >
          {activeTabData && (
            <p
              style={{
              ...
              data-testid={`tab-content-${activeTabData.id}`}
            >
...
function AccordionComponent({ props, styles, isSelected, onClick, isPreview, onTextChange, editingField, onEditField }: ComponentRenderProps) {
  const baseStyle = getBaseStyle(styles, isSelected, isPreview);
  const canEdit = !isPreview && onTextChange && onEditField;
  
  const items = props.accordionItems || [];
  const allowMultiple = props.allowMultiple === true || String(props.allowMultiple) === 'true';
  const defaultOpenIds = props.defaultOpen || [];
  
  const [openPanels, setOpenPanels] = useState<string[]>(defaultOpenIds);
  
  const textColor = styles.textColor || '#1a1a1a';
  const accentColor = styles.accentColor || '#4f46e5';
  const titleFontSize = styles.titleFontSize || '28px';
  const fontWeight = styles.fontWeight ? parseInt(styles.fontWeight) : 700;
  const fontFamily = styles.fontFamily || 'Inter, system-ui, sans-serif';
  ...
  return (
    <section style={{ ...baseStyle, fontFamily }} onClick={onClick}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }} data-testid="accordion-container">
          {items.map((item) => {
...
function ModalComponent({ props, styles, isSelected, onClick, isPreview }: ComponentRenderProps) {
  const [isOpen, setIsOpen] = useState(false);
  const baseStyle = getBaseStyle(styles, isSelected, isPreview);
  
  const triggerText = props.triggerText || 'Open Modal';
  const modalTitle = props.title || 'Modal Title';
  const modalContent = props.description || '';
  ...
  const sizeStyles: Record<string, React.CSSProperties> = {
    ...
    full: { maxWidth: '100%', width: '100%', height: '100%', margin: 0, borderRadius: 0 },
  };
  ...
  return (
    <section style={baseStyle} onClick={onClick}>
      <div style={{ display: 'flex', justifyContent: 'center', padding: styles.padding || '24px' }}>
        <button
      ...
      {isOpen && (
        <div
          ...
          style={{
            ...
            display: 'flex',
            alignItems: modalSize === 'full' ? 'stretch' : 'center',
            justifyContent: 'center',
            zIndex: 9999,
            padding: modalSize === 'full' ? 0 : '20px',
          }}
        ...
        >
          <div
            style={{
              ...
              borderRadius: modalSize === 'full' ? 0 : '12px',
...
export default function ComponentRenderer({ component, isSelected = false, onClick, isPreview = false, websiteId, pages, onTextChange, editingField, onEditField, onImageResize, onStyleChange, onHover }: RenderProps) {
  ...
  const renderComponent = () => {
    switch (component.type) {
      ...
      case 'accordion':
        return <AccordionComponent {...commonProps} />;
      case 'modal':
