...
The builder follows a "single source of truth" pattern where `builder_state` (pure JSON) drives rendering across all contexts.

### Core State Structure (BuilderStateData)
```typescript
{
  version: number,              // Schema version for migrations
  siteMetadata: {               // SEO and site info
    title, description, language, favicon, openGraph
  },
  navigation: {                 // Header/footer rendered from this
    header: { logo, logoText, links[], showCta, ctaText, ctaLink },
    footer: { copyright, links[], socials }
  },
  theme: {                      // Global styling
    colors: { primary, secondary, accent, background, surface, text, textMuted },
    fonts: { heading, body },
    spacing: { sectionPadding, containerMaxWidth },
    borderRadius
  },
  pages: [{                     // Array of page definitions
    id, name, path, title,
    components: [{              // Sections on the page
      id, type, props, styles, visibility: { desktop, tablet, mobile }
    }]
  }],
  activePage: string
}
...
- **Location**: `client/src/components/builder/blocks.tsx`
- **Components**: Hero, Features, CTA, Testimonials, Text+Image, Image Slider, Pricing, Contact Form
- **Header/Footer**: Rendered from `navigation` config, not as page components

### WebsiteRenderer
- **Location**: `client/src/components/builder/website-renderer.tsx`
- **Usage**: Builder preview, live preview page, future published sites
- **Device-agnostic**: Container width determines responsive layout (desktop: 100%, tablet: 768px, mobile: 375px)
- **Visibility**: Components can be hidden per device via `visibility` object

### Data Flow
1. `builder_state` stored as JSONB in database
2. Backend migration function transforms legacy state to new format
3. Frontend loads state, passes to WebsiteRenderer
4. All edits update state, which re-renders preview instantly
5. Save writes state back to database

## Recent Changes

- **2024-12-24**: Implemented single-source-of-truth builder architecture with shared renderers
- **2024-12-24**: Extended BuilderStateData with siteMetadata, navigation, theme, and device visibility
