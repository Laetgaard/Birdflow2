...
### Component System
A registry-based component system for the website builder defining 20 component types with editable properties. Includes a `ComponentRenderer` and `PropertiesPanel`. Per-component motion is configurable (effect, trigger, duration, delay, easing, distance, repeat, hover presets, box-level stagger for custom components) through a shared data-driven spec in `shared/motion.ts`, honored identically by builder preview and published sites and respecting `prefers-reduced-motion` on both.
...
Each custom component also carries an **editable schema** (`props.customSchema`): named Danish fields (text/image/link/colour/style-group/repeater) bound to tree nodes, so customers edit "Overskriften" instead of raw boxes. The properties panel shows these fields first ("Felter") with the raw node editor behind an "Avanceret" toggle; repeaters support add/remove/reorder with canvas-selectable items. Every edit — panel fields AND inline canvas editing — routes through `applySemanticEdit` in `shared/customComponents.ts`, so there is exactly one editing path. Components without a stored schema get one inferred at read time (`effectiveEditableSchema`); only stored schemas restrict inline editing to bound nodes. The publisher ignores `customSchema`.

Reusable SVG illustrations live in a per-website `svg_assets` store (`shared/svgAssets.ts`, `server/svgAssetStore.ts`; deduped by content hash): svg nodes reference them via `svgAssetId` plus optional per-slot colour overrides (`svgColors` — concrete colours or `{color.*}` design-token refs bound to the brand guide). Inline svg markup is extracted into assets at every builder-state save (PATCH route and `builderStateWriter`); the builder canvas resolves references at render time from a fetched asset map, the publisher inlines them at generation time before the usual sanitize. If storing an asset fails, the editor saves the markup inline in the node instead (never stranded). The component library carries searchable metadata — description, category (`hero/sektion/kort/cta/galleri/dekoration/andet`), tags, deterministic wireframe thumbnails, origin (customer/AI), version — with palette search + category filters and structural duplicate detection (`treeSignature`) that warns before saving a near-identical component; the AI saves reusable sections with the same metadata via `saveToLibrary`.
...
AI-powered website modification through structured JSON mutations supporting "Build Mode", "Thinking Mode", and "Design Analysis Mode". It includes "Creative Mode" (full CSS freedom) and "Safe Mode" (restricted styling), along with undo/redo functionality. Mutations cover components, pages, global styles, style presets, section-based composition, custom components (`add_custom_component` / `update_custom_component` — data trees only, optional library save; the AI emits an editable-field schema validated server-side against the tree, else one is inferred, and functional bindings — executable hrefs, scripts/forms in SVG — are rejected as visual-only with a steer toward the trusted booking/contact/pricing sections), brand-guide edits (`update_brand_guide`, deep partial merge with optional design-token application), and site structure (`reorder_pages`, `update_navigation`, `update_site_chrome` — stored menu, page roles/SEO, one shared header/footer with per-page opt-out; the "Sider" tab edits the same model). The AI panel (`AIBuilderPanel.tsx`) is fully in Danish.
...
### Business Facts & Claims Policy
Every website carries a customer-owned `builderState.businessContext` ("Forretningsfakta": name, industry, description, audience, location, services, conversion goals, facts with a protected/verbatim flag, forbidden claims). Onboarding derives it from interview answers (`deriveBusinessContext` — existing customer edits always win); customers edit it in the builder's Brand tab (`BusinessFactsPanel.tsx`); the AI has no mutation that can write it. Every AI run receives the facts as a prompt block in the site's language (`buildBusinessContextPrompt` in one-shot builder, agent, plan agent, architect plan/build, onboarding). Enforcement is deterministic and server-side (`server/claimRules.ts`), not prompt-only: bilingual (da+en) detectors for prices, statistics, testimonials/ratings, qualifications, credentials and outcome promises check every AI mutation at the `validateMutation` choke point (and the `/ai/apply` replay route) against an evidence pool = business facts + current site copy, so rephrasing or moving existing content is allowed but new unbacked claims are refused with a message naming the claim and pointing to Brand → Forretningsfakta. Whole generated states (onboarding builds, architect rebuilds, fallbacks) are scrubbed against the facts alone (`scrubStateClaims`) — unbacked testimonial/stat/pricing items are dropped (empty sections removed), claim sentences excised, and each removal reported in the build report. `forbiddenClaims` are refused even if factually backed.
