...
## Client resume model
The server owns resume: one endpoint returns the authoritative stage from the persisted generation/decision/payment dimensions, and the wizard follows it instead of inferring from localStorage. Only pre-generation wizard answers may live client-side. Draft AI websites are reused by create-website (`mode: "ai"`), so refreshes never pile up duplicate sites. See [onboarding decision flow](onboarding-decision-flow.md) for the post-generation half.
