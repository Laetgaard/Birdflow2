...
export function generateShopPage(): string {
  return `'use client';
...
export default function ShopPage() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: theme.backgroundColor }}>
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: theme.backgroundColor, color: '#1a1a1a' }}>
      {/* Header */}
      <header style={{ padding: '20px 24px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 700 }}>Shop</h1>
        <button 
          onClick={() => setShowCheckout(!showCheckout)} 
          style={{ padding: '10px 20px', backgroundColor: theme.primaryColor, color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}
        >
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
        {/* Checkout Panel */}
        {showCheckout && (
          <div style={{ marginBottom: '40px', padding: '24px', backgroundColor: '#f8f9fa', borderRadius: '12px' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '20px' }}>Your Cart</h2>
            
            {orderStatus === 'success' ? (
              <div style={{ textAlign: 'center', padding: '20px' }}>
                <p style={{ color: '#22c55e', fontSize: '18px', fontWeight: 600 }}>Order submitted successfully!</p>
                <p style={{ marginTop: '8px', opacity: 0.7 }}>We'll contact you shortly to confirm your order.</p>
              </div>
            ) : cart.length === 0 ? (
                ...
                {cart.map(item => (
                  <div key={item.product.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #eee' }}>
                    <div>
                      ...
                      <p style={{ opacity: 0.7, fontSize: '14px' }}>Qty: {item.quantity} x \${parseFloat(item.product.price).toFixed(2)}</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                      <span style={{ fontWeight: 600 }}>\${(parseFloat(item.product.price) * item.quantity).toFixed(2)}</span>
                      <button onClick={() => removeFromCart(item.product.id)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}>Remove</button>
                    </div>
                  </div>
                ))}
                
                <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '2px solid #ddd' }}>
                  <p style={{ fontSize: '20px', fontWeight: 700, textAlign: 'right' }}>Total: \${cartTotal.toFixed(2)}</p>
                </div>

                <form onSubmit={submitOrder} style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <input 
                    type="text" 
                    placeholder="Your Name" 
                  ...
                  <button 
                    ...
                    style={{ padding: '14px 24px', backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: 600, cursor: 'pointer' }}
                  >
                    {orderStatus === 'loading' ? 'Submitting...' : 'Request to Buy'}
                  </button>
                  {orderStatus === 'error' && <p style={{ color: '#ef4444', textAlign: 'center' }}>Something went wrong. Please try again.</p>}
                </form>
              </>
            )}
          </div>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            <p style={{ fontSize: '18px', opacity: 0.7 }}>No products available yet</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px' }}>
            {products.map(product => (
              <div key={product.id} style={{ backgroundColor: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
                {product.image_url ? (
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  {product.description && <p style={{ fontSize: '14px', opacity: 0.7, marginBottom: '16px', lineHeight: 1.5 }}>{product.description}</p>}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '22px', fontWeight: 700, color: theme.primaryColor }}>\${parseFloat(product.price).toFixed(2)}</span>
                    <button 
                      onClick={() => addToCart(product)}
                      disabled={product.stock === 0}
                      style={{ 
                        padding: '10px 20px', 
                        backgroundColor: product.stock === 0 ? '#ccc' : theme.primaryColor, 
                        color: '#fff', 
                        border: 'none', 
                        borderRadius: '8px', 
                  ...
                  {product.stock > 0 && product.stock < 10 && (
                    <p style={{ fontSize: '12px', color: '#f59e0b', marginTop: '8px' }}>Only {product.stock} left in stock</p>
                  )}
                </div>
              </div>
            ))}
